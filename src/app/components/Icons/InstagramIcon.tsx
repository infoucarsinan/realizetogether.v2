import React from "react";

function InstagramIcon(): JSX.Element {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="28.87"
      height="28.87"
      viewBox="0 0 28.87 28.87"
    >
      <defs>
        <linearGradient
          id="linear-gradient"
          x1="-0.064"
          y1="1.055"
          x2="1.114"
          y2="-0.105"
          gradientUnits="objectBoundingBox"
        >
          <stop offset="0" stop-color="#fed576" />
          <stop offset="0.26" stop-color="#f47133" />
          <stop offset="0.61" stop-color="#bc3081" />
          <stop offset="1" stop-color="#4c63d2" />
        </linearGradient>
      </defs>
      <g id="Layer_1" data-name="Layer 1">
        <rect
          width="28.87"
          height="28.87"
          rx="6.48"
          fill="url(#linear-gradient)"
        />
        <g id="_Group_">
          <path
            d="M10,5h9c.2.1.5.1.7.2a4.78,4.78,0,0,1,3.8,3.3,8,8,0,0,1,.3,1.5v8.8a6.94,6.94,0,0,1-1.2,3.1,5.51,5.51,0,0,1-4.5,1.9H10.6a5.49,5.49,0,0,1-3.7-1.2A5.51,5.51,0,0,1,5,18.14v-7a7.57,7.57,0,0,1,.1-1.5,4.9,4.9,0,0,1,3.8-4.3ZM6.9,14.5v3.9a3.42,3.42,0,0,0,3.7,3.7q3.9.15,7.8,0c2.3,0,3.6-1.4,3.7-3.7q.15-3.9,0-7.8a3.52,3.52,0,0,0-3.7-3.7q-3.9-.15-7.8,0a3.42,3.42,0,0,0-3.7,3.7Z"
            fill="#fff"
          />
          <path
            d="M9.64,14.54a4.91,4.91,0,0,1,4.9-4.9,5,5,0,0,1,4.9,4.9,4.91,4.91,0,0,1-4.9,4.9,5,5,0,0,1-4.9-4.9Zm4.9-3.1a3.05,3.05,0,1,0,3,3,3,3,0,0,0-3-3Z"
            fill="#fff"
          />
          <path
            d="M18.34,9.44a1.16,1.16,0,0,1,1.2-1.2,1.29,1.29,0,0,1,1.2,1.2,1.2,1.2,0,1,1-2.4,0Z"
            fill="#fff"
          />
        </g>
      </g>
    </svg>
  );
}

export default InstagramIcon;
